...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 7005262649]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 Whisper",
            description=f"@EsproChatBot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**📍Usage:**\n\n@EsproChatBot (Target Username or ID) (Your Message).\n\n**Example:**\n@EsproChatBot @username I Wanna Phuck You"),
            thumb_url="https://te.legra.ph/file/3eec679156a393c6a1053.jpg",
            reply_markup=switch_btn
        )
    ]
